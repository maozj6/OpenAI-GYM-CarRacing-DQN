...
class MDNRNN():
  def __init__(self, hps, gpu_mode=True, reuse=False):
    self.hps = hps
    with K.variable_scope('mdn_rnn', reuse=reuse):
      if not gpu_mode:
  ...
  def build_model(self, hps):
    ...
    # cell_fn = tf.nn.rnn_cell.LayerNormBasicLSTMCell
    cell_fn=K.nn.rnn_cell.LSTMCell
    ...
    self.input_x = K.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len, INWIDTH])
    self.output_x = K.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len, OUTWIDTH])
    ...
    with K.variable_scope('RNN'):
      output_w = K.get_variable("output_w", [self.hps.rnn_size, NOUT])
      output_b = K.get_variable("output_b", [NOUT])

    output, last_state = K.nn.dynamic_rnn(cell, actual_input_x, initial_state=self.initial_state,
                                           time_major=False, swap_memory=True, dtype=tf.float32, scope="RNN")

    output = tf.reshape(output, [-1, hps.rnn_size])
    output = K.nn.xw_plus_b(output, output_w, output_b)
    ...
    if self.hps.is_training == 1:
      ...
      optimizer = K.train.AdamOptimizer(self.lr)
